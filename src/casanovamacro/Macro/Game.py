...
def set_top_menu(state=True): # INIT MEAN  UNCOVER HIDE TOP MENU BUTTON FROM EQUIP DURABILITY INDICATOR
    ...
    while True:
        if check_image_existance(TOP_MENU_ON_STATE if state else TOP_MENU_OFF_STATE , confidence=0.98):
            print(f"Test:Game:TOP MENU {state}")
            return True
        
        elif  check_image_existance(TOP_MENU_ON_STATE if state else TOP_MENU_OFF_STATE , confidence=0.98) == False:
            click(1126,46)
            sleep(1)
         
        #PREVENT ERROR BECAUSE EQUIPMENT BROKEN INDICATOR COVERING THE ICON
        elif check_image_existance(TOP_MENU_ON_STATE) == False and  check_image_existance(TOP_MENU_OFF_STATE) == False:
...
def talk_to_npc_by_map(npc_name, npc_index=0, allow_afk=False): 
    ...
    print(f"Macro:Base:Talking to {npc_link}")
    if allow_afk == False: 
